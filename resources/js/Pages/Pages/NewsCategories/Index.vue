...
const save = () => {
    if (form.id) {
        form.post(route('control.pages.news-categories.update', form.id), {
            preserveScroll: true,
            onSuccess: () => {
        ...
        form.post(route('control.pages.news-categories.store'), {
            preserveScroll: true,
            onSuccess: () => {
                showModal.value = false;
                form.reset();
...
const callDelete = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('control.pages.news-categories.destroy', id), {
                preserveScroll: true,
            });
        }
    });
...
const callRestore = (category) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.post(route('control.pages.news-categories.restore', category.id), {}, {
                preserveScroll: true,
            });
        }
    });
...
const callForceDelete = (category) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('control.pages.news-categories.force_delete', category.id), {
                preserveScroll: true,
            });
        }
    });
